...
with open(os.getcwd()+"/files/nyphil.json") as f:
    df = json.load(f)
...
for program in df['programs']:
    if(program['id'] == '00646b9f-fec7-4ffb-9fb1-faae410bd9dc-0.1'):
        program_df = json_normalize(program)
        id = program['id']
        df3.append(program_df)
        for work in program['works']:
            work_df = json_normalize(work)
            work_df['id'] = id
            ID = work_df['ID']
            df1.append(work_df)
            if (len(work['soloists']) == 0):
                ...
                for soloist in work['soloists']:
                    solo_df = json_normalize(soloist)
...
for program in df['programs']:
    if(program['id'] == '00646b9f-fec7-4ffb-9fb1-faae410bd9dc-0.1'):
        ...
        for concert in program['concerts']:
            concert_df = json_normalize(concert)
...
p_df.to_csv(os.getcwd()+"/files/p_df.csv", index=False)
w_df.to_csv(os.getcwd()+"/files/w_df.csv", index=False)
s_df.to_csv(os.getcwd()+"/files/s_df.csv", index=False)
c_df.to_csv(os.getcwd()+"/files/c_df.csv", index=False)
out_3.to_csv(os.getcwd()+"/files/out.csv", index=False)
